...
Starting a match now presents the selected backdrop as a cinematic title card before the board and HUD arrive. Each theme owns its title face and color in the theme catalog. Pirate uses Pirata One, Space Age uses Orbitron, Kitchen Witchery uses Berkshire Swash, Prairie Homestead uses Rye, and Velvet & Leather uses Bodoni Moda; these five faces are bundled under the SIL Open Font License with their license files. Wizardry and Gothic Horror use the existing project display faces.
...
Blank selection and rune trading use compact modal cards that inherit the active world's panel and accent colors. Both have explicit headings, guidance, cancellation, keyboard-focus states, and bounded sizing; the trade card also provides a live selection count and keeps confirmation disabled until at least one rune is chosen.
